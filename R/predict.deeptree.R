...
predict.deeptree= function (object, newData, treeLeaves = NA, stackPred = NA, ...)
{
  ...
  newDataSplit <- lapply(treeLeavesGroup[treeLeavesGroup %in%
                                           unique(treeLeaves)], function(tp) {
  ...
  modelGroup = modelGroup[ treeLeavesGroup %in% unique(treeLeaves)]

  predlist <- lapply(1:length(unique(treeLeaves)), function(mg) {
    if (!useStackPred[[mg]]) {
      if (class(modelGroup[[mg]]) == "deepnet") {
        if (length(xCols[[mg]]) == 1) {
          predict.deepnet(modelGroup[[mg]], data.frame(newDataSplit[[mg]][,
                                                                          xCols[[mg]]]))
        }
        else {
